...
architecture struct of ICS_EDU_RV32I_SC is
  ...
  component control_unit
    port(
      ...
      LoadType   : out std_ulogic_vector(2 downto 0)
    );
  ...
  component datapath
    ...
    port(
      ...
      ram_regs   : out regs_ram;
      ram_dmem   : out dmem_ram;
      LoadType   : in  std_ulogic_vector(2 downto 0)
    );
  end component datapath;
    
  signal ALUSrc, RegWrite, MemWrite, Zero       : STD_ULOGIC;
  signal Instr                                  : STD_ULOGIC_VECTOR(31 downto 0);
  signal ResultSrc, PCSrc                       : STD_ULOGIC_VECTOR(1 downto 0);
  signal ImmSrc                                 : STD_ULOGIC_VECTOR(IMM_SRC_SIZE-1 downto 0);
  signal ALUControl                             : STD_ULOGIC_VECTOR(ALU_CTRL_SIZE-1 downto 0);
  signal LoadType                               : std_ulogic_vector(2 downto 0);

begin

  c: control_unit port map(Instr(6 downto 0), Instr(14 downto 12), Instr(30), Zero, ResultSrc, MemWrite, PCSrc, ALUSrc, RegWrite, ImmSrc, ALUControl, LoadType);
  dp: datapath generic map (TEXT_SEGMENT, DATA_SEGMENT, REGISTERS) port map(clk, reset, ResultSrc, PCSrc, ALUSrc, RegWrite, MemWrite, ImmSrc, ALUControl, Zero, Instr, ram_regs, ram_dmem, LoadType);
